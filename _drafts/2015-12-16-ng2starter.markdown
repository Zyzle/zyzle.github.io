...
I'm lets take a look at the code then. The app itself is made up of only 4 files (everything else in the repository is configuration):
...
Lets go through these one at a time. `index.html`, as you might have guessed, is the main application page. This is some simple HTML containing the Angular 2 app main tag and the `script` tags to load the required polyfills/shims and the Angular 2 app itself.
...
You may have noticed the custom `<hello-app>` tag. Angular 2 apps are built around web components, this tag is the component we will define in our application.
...
The code in our `entry` module will be executed when this bundle is loaded into the browser through the `<script>` tag. We've already gone over the `bootstrap.ts` file contents so lets take a look at the only other TypeScript file in our app `hello.ts`.
...
Here (finally) we have the definition for the `<hello-app>` component. You'll notice our Angular 2 component is simply a TypeScript class with the `@Component` decorator applied to it.
